...
## 1. Bare `exit 1` inside the backgrounded run — [lib/rep_apps.bash:142](../lib/rep_apps.bash#L142)
...
## 2. `Restore_immich_DB` wipes the DB before checking the target app is running — [lib/immich_db.bash:69-88](../lib/immich_db.bash#L69-L88)
...
## 3. `Control_app` cannot see a FAILED job — [lib/rep_apps.bash:15-20](../lib/rep_apps.bash#L15-L20)
...
## 4. Mounted-check passes when the dataset doesn't exist — [lib/rep_filesystems.bash:103-104](../lib/rep_filesystems.bash#L103-L104)
...
## 5. Impacted-dataset discovery ignores the property VALUE — [lib/rep_filesystems.bash:116-123](../lib/rep_filesystems.bash#L116-L123)
...
## 6. `Background_error` inside `$( )` cannot abort the run → empty-pool guards
...
## 7. Docker container matched by substring — [lib/docker.bash:16](../lib/docker.bash#L16) and [lib/docker.bash:40](../lib/docker.bash#L40)
...
## 8. Target VM state never checked before `vm.delete` — [lib/rep_vms.bash:736](../lib/rep_vms.bash#L736)
...
## 9. `mkdir -p` on the target runs even with `--test` — [lib/rep_vms.bash:522](../lib/rep_vms.bash#L522)
...
## 10. `PERFORM_ROLLUP` set twice — [lib/cli.bash:71-76](../lib/cli.bash#L71-L76)

`--subtask=all` assigns `PERFORM_ROLLUP="true"` on both line 71 and line 76.
Delete one of them. (Cosmetic.)

## 11. Echoed restore command drifted from the executed one — [lib/immich_db.bash:93-95](../lib/immich_db.bash#L93-L95)
...
1. `bash -n` on every touched file: `cli.bash`, `common.bash` (untouched),
   `docker.bash`, `immich_db.bash`, `rep_apps.bash`, `rep_filesystems.bash`,
   `rep_vms.bash`, `snap_rollup.bash`.
2. On a TrueNAS host: `./bin/sync_truenas_servers --test --task=master_to_backup`
   — must complete, banner and email as before, no empty-pool errors.
...
4. Item 5: confirm the impacted-datasets list printed in the log is identical
   to a pre-change run (unless you have `=false`-tagged datasets).
