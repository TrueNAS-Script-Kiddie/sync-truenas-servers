...
|---|---|---|---|
| [01-bug-fixes.md](01-bug-fixes.md) | 14 concrete bugs (11 original + 3 found via plan 08's investigation). 11 low-risk/easily-tested ones done and lint-verified (2026-07-03), 8 of them confirmed on both a `--test` and a real run; items 12/13 (diagnostic capture) and item 14 (a failed run left 6 production datasets unmounted for hours — fixed with a scoped `trap`, NOT a blind "remount everything") await a real occurrence to fully confirm; 3 deferred (Immich restore guard, dataset-selection query change, VM target-state check) | Low (done items); Medium (3 deferred) | Tackle the 3 deferred ones next |
